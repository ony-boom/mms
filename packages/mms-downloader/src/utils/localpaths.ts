...
export function getConfigFolder() {
  ...
  if (process.env.MMS_DOWNLOADER_DATA_DIR)
    return process.env.MMS_DOWNLOADER_DATA_DIR.replace(/\/*$/, "") + "/";
  ...
  else userdata += `mms-downloader${sep}`;
...
export function getMusicFolder() {
  ...
  if (process.env.MMS_DOWNLOADER_MUSIC_DIR)
    return process.env.MMS_DOWNLOADER_MUSIC_DIR.replace(/\/*$/, "") + "/";
  ...
  else musicdata += `MMS Downloader Music${sep}`;
